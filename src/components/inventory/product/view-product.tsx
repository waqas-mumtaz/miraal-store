...
export default function ProductDetail() {
  ...
  const handleReplenish = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      // Call API to replenish product
      const response = await fetch(`/api/products/${productId}/replenish`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        setReplenishError(data.error || 'Failed to replenish product');
        setIsReplenishing(false);
        return;
      }

      setReplenishSuccess("Product replenished successfully!");
  ...
  if (!product) {
    return (
      <div className="p-5 border border-gray-200 rounded-2xl dark:border-gray-800 lg:p-6">
        <div className="text-center">
          <h3 className="text-lg font-semibold text-gray-800 dark:text-white mb-2">Product Not Found</h3>
          <p className="text-gray-500 dark:text-gray-400 mb-4">The requested product could not be found.</p>
          <Button onClick={() => router.back()}>Go Back</Button>
        </div>
      </div>
    );
  ...
  const quantityStatus = getQuantityStatus(product.currentQuantity);
  ...
  return (
    <div className="p-5 border border-gray-200 rounded-2xl dark:border-gray-800 lg:p-6">
      ...
      <div className="grid grid-cols-1 gap-6 mt-6 lg:grid-cols-2">
        <div className="space-y-4">
          <div>
            ...
            <p className="text-sm text-gray-800 dark:text-white/90">{product.name}</p>
          </div>
          {product.sku && (
            <div>
              <Label>SKU</Label>
              <p className="text-sm text-gray-800 dark:text-white/90">{product.sku}</p>
            </div>
          )}
          {product.description && (
            <div>
              <Label>Description</Label>
              <p className="text-sm text-gray-800 dark:text-white/90">{product.description}</p>
            </div>
          )}
        </div>
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="flex items-center gap-2">
              <p className="text-sm text-gray-800 dark:text-white/90">{product.currentQuantity}</p>
          ...
          <div>
            ...
            <p className="text-sm text-gray-800 dark:text-white/90">{formatCurrency(product.unitCost)}</p>
          </div>
          <div>
            <Label>Total COG</Label>
            <p className="text-sm text-gray-800 dark:text-white/90">{formatCurrency(product.totalCOG)}</p>
          </div>
        </div>
      </div>
      ...
      {/* Linked Expenses */}
      {product.linkedExpenses && product.linkedExpenses.length > 0 && (
        <div className="mt-8">
          <h5 className="text-md font-semibold text-gray-800 dark:text-white/90 mb-4">
            Linked Expenses
          </h5>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {product.linkedExpenses.map((link) => (
      ...
      {product.replenishments && product.replenishments.length > 0 && (
        <div className="mt-8">
          <h5 className="text-md font-semibold text-gray-800 dark:text-white/90 mb-4">
            Replenishment History
          </h5>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {product.replenishments.map((replenishment) => (
