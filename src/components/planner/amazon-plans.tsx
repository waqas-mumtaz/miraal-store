...
export default function AmazonPlans() {
  ...
  const fetchPlans = async () => {
    try {
      ...
      if (!response.ok) {
        setError('Failed to load Amazon plans');
        setIsLoading(false);
        return;
      }
      ...
      console.error('Error fetching Amazon plans:', error);
      setError('Failed to load Amazon plans');
      setIsLoading(false);
    }
  };
  ...
  const handleDelete = async (planId: string) => {
    if (!confirm('Are you sure you want to delete this Amazon plan?')) {
      return;
    }
      ...
      <div className="p-6">
        <div className="flex items-center justify-center h-64">
          <div className="text-center">
            ...
            <p className="mt-4 text-gray-600">Loading Amazon plans...</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="p-6">
      ...
      {plans.length > 0 ? (
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {plans.map((plan) => (
                  <tr key={plan.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {plan.amazonDetails?.fulfillmentType || 'N/A'}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
        ...
        <div className="text-center py-12">
          ...
          <h3 className="mt-2 text-sm font-medium text-gray-900">No Amazon plans found</h3>
